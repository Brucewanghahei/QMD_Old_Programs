...
def current_sweep(inst, Current, current_first, current_second, current_lim, voltage_lim, wait_time, probe = '4'):
    ...
    while True:
        ...
        if length_whole >= 50:
            count = 50
            for i in range(item * 50, (item + 1) * 50):
                Str_Current = Str_Current + ', ' + str(Current[i])
        else:
            count = length_whole
            for i in range(item * 50, len(Current)):
                Str_Current = Str_Current + ', ' + str(Current[i])
...
def voltage_sweep(inst, Voltage, voltage_first, voltage_second, voltage_lim, current_lim, wait_time, probe = "4"):
...
class sweep_stop():
    ...
    def voltage_sweep_stop(self, inst, Voltage, voltage_first, voltage_second, voltage_lim, current_lim, wait_time, probe = "4", ui = False):
        length_first = len(voltage_first)
        length_second = len(voltage_second)
        length_whole = len(Voltage)
        time_difference = 0
...
if __name__ == '__main__':
    ...
    current_first = range(-10, 10, 1)
    current_descending = range(10, 0, -1)
    current_first = numpy.array(list(current_first), dtype = 'float')/1E6
    current_descending = numpy.array(list(current_descending), dtype = 'float')/1E6
    Current = numpy.append(current_first, current_descending)
    waitTime = 0.001
    ...
    voltage_lim = 1
    probe = 2
    data = current_sweep(inst, Current, current_first, current_lim, voltage_lim, waitTime, probe)
'''
'''if __name__ == '__main__':
    rm = visa.ResourceManager()
    rm.list_resources()
