...
class MyForm(QMainWindow):
    ...
    def VoltageSweep(self):
        ...
        if (self.ui.radioButton_software.isChecked() == True):
            ...
            num_steps = int(self.ui.lineEdit_steps_hard.text())
        ...
        if (self.ui.radioButton_software.isChecked() == True):
            ...
            voltage_steps = (start_voltage - 2 * first_voltage + 2 * second_voltage - end_voltage) / num_steps
    ...
    def CurrentSweep(self):
        ...
        if (self.ui.radioButton_software.isChecked() == True):
            ...
            current_steps = (start_current - 2 * first_current + 2 * second_current - end_current) / num_steps
    ...
    def run(self):
        ...
        if self.sweep == "VOLTAGE":
            if self.ui.radioButton_hardware.isChecked():
                voltage_values, current_values, resistance_first, resistance_second, resistance_third, scaled_best_fit_first, scaled_best_fit_second, scaled_best_fit_third, voltage_scale, current_scale, resistance_first_scale, resistance_second_scale, resistance_third_scale, time, length_first, length_second = voltage_sweep(self.inst, self.sweep_array, self.sweep_array_first, self.sweep_array_second, self.sweep_lim, self.read_lim, self.wait_time, self.probe)
            elif self.ui.radioButton_software.isChecked():
                self.sweep_stop = Sweep.sweep_stop()
                voltage_values, current_values, resistance_first, resistance_second, resistance_third, scaled_best_fit_first, scaled_best_fit_second, scaled_best_fit_third, voltage_scale, current_scale, resistance_first_scale, resistance_second_scale, resistance_third_scale, time, length_first, length_second = self.sweep_stop.voltage_sweep_stop(self.inst, self.sweep_array, self.sweep_array_first, self.sweep_array_second, self.sweep_lim, self.read_lim, self.wait_time, self.probe, self.ui)
            ...
            if self.ui.radioButton_hardware.isChecked():
                voltage_values, current_values, resistance_first, resistance_second, resistance_third, scaled_best_fit_first, scaled_best_fit_second, scaled_best_fit_third, voltage_scale, current_scale, resistance_first_scale, resistance_second_scale, resistance_third_scale, time, length_first, length_second = current_sweep(self.inst, self.sweep_array, self.sweep_array_ascending, self.sweep_lim, self.read_lim, self.wait_time, self.probe)
            else:
                self.sweep_stop = Sweep.sweep_stop()
    ...
    def addTable(self, resistance_first, resistance_first_scale, resistance_second, resistance_second_scale, resistance_third, resistance_third_scale):
        ...
        if self.sweep == "VOLTAGE":
            ...
            if (self.ui.radioButton_software.isChecked() == True):
                ...
                num_step = self.ui.lineEdit_steps_hard.text()
