...
class MyForm(QMainWindow):
    ...
    def __init__(self, parent = None):
        ...
        self.canvas_allplot_p = FigureCanvas(self.ui.mplwidget_allplot_p.figure)
        self.canvas_allplot_p.setParent(self.ui.widget_allplot_p)
        self.mpl_toolbar_allplot_p = NavigationToolbar(self.canvas_allplot_p, self.ui.widget_allplot_p)
        
        self.canvas_real_time_p = FigureCanvas(self.ui.mplwidget_real_time_p.figure)
        self.canvas_real_time_p.setParent(self.ui.widget_real_time_p)
        self.mpl_toolbar_real_time_p = NavigationToolbar(self.canvas_real_time_p, self.ui.widget_real_time_p)

        self.canvas_first_p = FigureCanvas(self.ui.mplwidget_first_p.figure)
        self.canvas_first_p.setParent(self.ui.widget_first_p)
        self.mpl_toolbar_first_p = NavigationToolbar(self.canvas_first_p, self.ui.widget_first_p)

        self.canvas_second_p = FigureCanvas(self.ui.mplwidget_second_p.figure)
        self.canvas_second_p.setParent(self.ui.widget_second_p)
        self.mpl_toolbar_second_p = NavigationToolbar(self.canvas_second_p, self.ui.widget_second_p)
        
        self.canvas_third_p = FigureCanvas(self.ui.mplwidget_third_p.figure)
        self.canvas_third_p.setParent(self.ui.widget_third_p)
        self.mpl_toolbar_third_p = NavigationToolbar(self.canvas_third_p, self.ui.widget_third_p)
        ...
        vbox_allplot_p = QVBoxLayout()
        vbox_allplot_p.addWidget(self.canvas_allplot_p)
        vbox_allplot_p.addWidget(self.mpl_toolbar_allplot_p)
        self.ui.widget_allplot_p.setLayout(vbox_allplot_p)
        
        vbox_real_time_p = QVBoxLayout()
        vbox_real_time_p.addWidget(self.canvas_real_time_p)
        vbox_real_time_p.addWidget(self.mpl_toolbar_real_time_p)
        self.ui.widget_real_time_p.setLayout(vbox_real_time_p)

        vbox_first_p = QVBoxLayout()
        vbox_first_p.addWidget(self.canvas_first_p)
        vbox_first_p.addWidget(self.mpl_toolbar_first_p)
        self.ui.widget_first_p.setLayout(vbox_first_p)

        vbox_second_p = QVBoxLayout()
        vbox_second_p.addWidget(self.canvas_second_p)
        vbox_second_p.addWidget(self.mpl_toolbar_second_p)
        self.ui.widget_second_p.setLayout(vbox_second_p)
        
        vbox_third_p = QVBoxLayout()
        vbox_third_p.addWidget(self.canvas_third_p)
        vbox_third_p.addWidget(self.mpl_toolbar_third_p)
        self.ui.widget_third_p.setLayout(vbox_third_p)
        ...
        self.ui.mplwidget_allplot_p = self.canvas_allplot_p
        self.ui.mplwidget_real_time_p = self.canvas_real_time_p
        self.ui.mplwidget_first_p = self.canvas_first_p
        self.ui.mplwidget_second_p = self.canvas_second_p
        self.ui.mplwidget_third_p = self.canvas_third_p
        ...
        self.connect(self.ui.pushButton_Pulse, SIGNAL("clicked()"), self.Pulse)
        ...
        self.connect(self.ui.pushButton_Select_Directory_p, SIGNAL('clicked()'), self.select_check_p)
        self.connect(self.ui.pushButton_Save, SIGNAL('clicked()'), self.save)
        self.connect(self.ui.pushButton_Save_p, SIGNAL('clicked()'), self.save_p)
        ...
        self.connect(self.ui.pushButton_browse_p, SIGNAL('clicked()'), self.Browse_p)
        self.connect(self.ui.pushButton_check, SIGNAL('clicked()'), self.Check)
        self.connect(self.ui.pushButton_check_p, SIGNAL('clicked()'), self.Check_p)
        self.connect(self.ui.pushButton_Open, SIGNAL('clicked()'), self.Open)
        self.connect(self.ui.pushButton_Open_p, SIGNAL('clicked()'), self.Open_p)
        ...
        self.connect(self.ui.comboBox_previous, SIGNAL('currentIndexChanged (int)'), self.previous)
        self.connect(self.ui.comboBox_time, SIGNAL('currentIndexChanged (int)'), self.ptime)
    ...
    def set_data(self, return_data):
        # 0voltage_values, 1current_values, 2resistance_first, 3resistance_second, 4resistance_third, 5caled_best_fit_first, 6scaled_best_fit_second, 7scaled_best_fit_third, 8voltage_scale
        # 9current_scale, 10resistance_first_scale, 11resistance_second_scale, 12resistance_third_scale, 13time, 14length_first, 15length_second, 16data
        ...
        self.p_data = return_data[16]
        #data[39]
        self.p_data.append(self.Time)
        self.first_Time = self.Time[:self.length_first]
        #data[40]
        self.p_data.append(self.first_Time)
        ...
        #data[41]
        self.p_data.append(self.second_Time)
        ...
        #data[42]
        self.p_data.append(self.third_Time)
        ...
        self.previous_data.append(self.p_data)
        self.Ready_plot()
    
    def Ready_plot(self):
        self.ui.comboBox_previous.clear()
        self.ui.comboBox_time.clear()
        for i in range(0, len(self.previous_data)):
            self.ui.comboBox_previous.addItem(self.previous_data[i][0])
            self.ui.comboBox_time.addItem(self.previous_data[i][1])
    
    def Plot(self):
        item = self.ui.comboBox_previous.currentIndex()
        
        self.reset_plot_allplot_p()
        self.axes_allplot_p.plot(self.previous_data[item][2], self.previous_data[item][3], marker = '.', linestyle = '')
        self.axes_allplot_p.set_title('\n'.join(wrap('Voltage vs. Current with All Points')))
        self.axes_allplot_p.set_xlabel('Voltage (' + self.previous_data[item][4] + ')')
        self.axes_allplot_p.set_ylabel('Current (' + self.previous_data[item][5] + ')')
        self.ui.mplwidget_allplot_p.draw()
        
        self.ui.lineEdit_bfr_first_p.setText(self.previous_data[item][6])
        self.ui.label_resistance_unit_first_p.setText(self.previous_data[item][7])
        self.ui.lineEdit_bfr_second_p.setText(self.previous_data[item][8])
        self.ui.label_resistance_unit_second_p.setText(self.previous_data[item][9])
        self.ui.lineEdit_bfr_third_p.setText(self.previous_data[item][10])
        self.ui.label_resistance_unit_third_p.setText(self.previous_data[item][11])
        
        self.reset_plot_first_p()
        self.axes_first_p.plot(self.previous_data[item][13], self.previous_data[item][14], marker = '.', linestyle = '')
        self.axes_first_p.plot(self.previous_data[item][13], self.previous_data[item][12])
        self.axes_first_p.set_title('\n'.join(wrap('Voltage vs. Current with the first period voltage')))
        self.axes_first_p.set_xlabel('Voltage (' + self.previous_data[item][15] + ')')
        self.axes_first_p.set_ylabel('Current (' + self.previous_data[item][16] + ')')
        self.ui.mplwidget_first_p.draw()
        
        self.reset_plot_second_p()
        self.axes_second_p.plot(self.previous_data[item][18], self.previous_data[item][19], marker = '.', linestyle = '')
        self.axes_second_p.plot(self.previous_data[item][18], self.previous_data[item][17])
        self.axes_second_p.set_title('\n'.join(wrap('Voltage vs. Current with the second period voltage' )))
        self.axes_second_p.set_xlabel('Voltage (' + self.previous_data[item][20] + ')')
        self.axes_second_p.set_ylabel('Current (' + self.previous_data[item][21] + ')')        
        self.ui.mplwidget_second_p.draw()
        
        self.reset_plot_third_p()
        self.axes_third_p.plot(self.previous_data[item][23], self.previous_data[item][24], marker = '.', linestyle = '')
        self.axes_third_p.plot(self.previous_data[item][23], self.previous_data[item][22])
        self.axes_third_p.set_title('\n'.join(wrap('Voltage vs. Current with the third period voltage' )))
        self.axes_third_p.set_xlabel('Voltage (' + self.previous_data[item][25] + ')')
        self.axes_third_p.set_ylabel('Current (' + self.previous_data[item][26] + ')')
        self.ui.mplwidget_third_p.draw()
        
        self.reset_plot_real_time_p()
        self.axes_real_time_p.plot(self.previous_data[item][27], self.previous_data[item][28])
        self.axes_real_time_p.set_title('\n'.join(wrap('Voltage vs.Real Time')))
        self.axes_real_time_p.set_xlabel('Time (s)')
        self.axes_real_time_p.set_ylabel('Voltage (' + self.previous_data[item][29] + ')')
        self.ui.mplwidget_real_time_p.draw()
        
        self.ui.groupBox_save_p.setEnabled(True)
        
    def reset_plot_allplot_p(self):
        self.ui.mplwidget_allplot_p.figure.clear()
        self.axes_allplot_p = self.ui.mplwidget_allplot_p.figure.add_subplot(111)
    
    def reset_plot_first_p(self):
        self.ui.mplwidget_first_p.figure.clear()
        self.axes_first_p = self.ui.mplwidget_first_p.figure.add_subplot(111)
    
    def reset_plot_second_p(self):
        self.ui.mplwidget_second_p.figure.clear()
        self.axes_second_p = self.ui.mplwidget_second_p.figure.add_subplot(111)

    def reset_plot_third_p(self):
        self.ui.mplwidget_third_p.figure.clear()
        self.axes_third_p = self.ui.mplwidget_third_p.figure.add_subplot(111)
        
    def reset_plot_real_time_p(self):
        self.ui.mplwidget_real_time_p.figure.clear()
        self.axes_real_time_p = self.ui.mplwidget_real_time_p.figure.add_subplot(111)
    
    def Previous(self):
        name_number = self.ui.comboBox_previous.currentIndex()
        if name_number != 0:
            name_number -= 1
            self.ui.comboBox_previous.setCurrentIndex(name_number)
            self.ui.comboBox_time.setCurrentIndex(name_number)
            self.Plot()
    
    def Next(self):
        name_number = self.ui.comboBox_previous.currentIndex()
        if name_number != len(self.previous_data) - 1:
            name_number += 1
            self.ui.comboBox_previous.setCurrentIndex(name_number)
            self.ui.comboBox_time.setCurrentIndex(name_number)
            self.Plot()
    ...
    def Pulse(self):
        self.ui.pushButton_Pulse.setEnabled(False)
        self.ui.pushButton_Continue.setEnabled(True)
        self.ui.lineEdit_condition.setText('Stopped. Click "continue" to go on.')
        self.collect_data_thread.pulse_continue()
    
    def Stop(self):
        self.ui.lineEdit_condition.setText('Stopped and ready to plot.')
    ...
    def ptime(self):
        name_number = self.ui.comboBox_time.currentIndex()
        self.ui.comboBox_previous.setCurrentIndex(name_number)
        self.ui.comboBox_time.setCurrentIndex(name_number)
    ...
    def Continue(self):
        ...
        self.ui.pushButton_Pulse.setEnabled(True)
    ...
    def judge_measurement(self):
        ...
        self.ui.tabWidget_plot.setEnabled(True)
    ...
    def VoltageSweep(self):
        ...
        self.collect_data_thread.input(self.visa_chosen, Voltage, voltage_first, voltage_second, voltage_third,
    ...
    def select_check_p(self):
        self.ui.pushButton_Save_p.setEnabled(True)
        self.googledrive_directory_p = str(self.ui.lineEdit_GoogleDrive_p.text())
        self.namefolder_p = str(self.ui.comboBox_Name_Folder_p.currentText())
        if self.namefolder_p == 'None':
            self.ui.lineEdit_save_condition_p.setText('Please choose a name folder to save.')
        else:
            self.date_p = '%s-%s-%s' % (now.year, now.month, now.day)
            self.ui.lineEdit_save_condition_p.setText("Save file to Google Drive\\" + self.namefolder_p + "\Date" + '\\' + self.date_p)
            self.ui.groupBox_File_Type_p.setEnabled(True)
            self.ui.groupBox_Filename_p.setEnabled(True)
            self.ui.label_46.setEnabled(True)
            self.ui.textEdit_comment_p.setEnabled(True)
    
    def Browse(self):
        prev_dir = 'C:\\'
        self.fileDir = QFileDialog.getOpenFileName(self, 'Select Google Drive File to Open:', prev_dir)
        if self.fileDir != '':
    ...
    def Browse_p(self):
        prev_dir = 'C:\\'
        self.fileDir_p = QFileDialog.getOpenFileName(self, 'Select Google Drive File to Open:', prev_dir)
        if self.fileDir_p != '':
            self.file_list_p = str(self.fileDir_p).split('/')
            for i in range(0, len(self.file_list_p) - 1):
                if i < len(self.file_list_p) - 1:
                    self.open_dir_p += self.file_list_p[i] + '\\'
                elif i == len(self.file_list_p) - 1:
                    self.open_dir_p += self.file_list_p[i]
            self.fileDir_p.replace('/', '\\')
            self.ui.lineEdit_GoogleDrive_p.setText(self.fileDir_p)
            self.ui.lineEdit_save_condition_p.setText('Open Google Drive User Folder')
        else:
            self.ui.lineEdit_GoogleDrive_p.setText('None')
            self.ui.lineEdit_save_condition_p.setText('Failed to Read File')
    ...
    def Check_p(self):
        file_list2 = []
        file_list2 = str(self.ui.lineEdit_GoogleDrive_p.text()).split('\\')
        user_folder = file_list2[len(file_list2) - 1]
        if os.path.exists(self.ui.lineEdit_GoogleDrive_p.text()) == False:
            self.ui.lineEdit_save_condition_p.setText('Incorrect Google Drive Directory.')
        else:
            if user_folder.upper() != '03 User Accounts'.upper():
                self.ui.lineEdit_save_condition_p.setText('Please click browse to the "03 User Accounts" folder')
            else:
                self.ui.label_24.setEnabled(True)
                self.ui.comboBox_Name_Folder_p.setEnabled(True)
                self.ui.pushButton_Select_Directory_p.setEnabled(True)
    ...
    def select_type_p(self):
        if self.ui.radioButton_csv_p.isChecked():
            self.type_p = '.csv'
            self.divide_p = ','
            self.form_p = ''
        elif self.ui.radioButton_txt_p.isChecked():
            self.type_p = '.txt'
            self.divide_p = '\t'
            self.form_p = '                         '
    ...
    def select_name_p(self):
        from datetime import datetime
        now = datetime.now()
        if self.ui.radioButton_Timename_p.isChecked():
            self.date_p = '%s-%s-%s' % (now.year, now.month, now.day)
            self.current_time_p = '%s.%s.%s' % (now.hour, now.minute, now.second)
            self.date_and_time_p = self.date_p + ' ' + self.current_time_p
            self.file_name_p = str(self.date_and_time_p)
        elif self.ui.radioButton_Custom_Name_p.isChecked():
            self.file_name_p = str(self.ui.lineEdit_Custom_Name_p.text())
    
    def enable_custom_name(self):
        self.ui.lineEdit_Custom_Name.setEnabled(True)
        
    def enable_custom_name_p(self):
        self.ui.lineEdit_Custom_Name_p.setEnabled(True)
        
    def disable_custom_name(self):
        self.ui.lineEdit_Custom_Name.setEnabled(False)
        
    def disable_custom_name_p(self):
        self.ui.lineEdit_Custom_Name_p.setEnabled(False)
    ...
    def save_p(self):
        ...
        self.select_name_p()
        if self.file_name_p == '':
            self.ui.lineEdit_save_condition_p.setText('Please enter a valid file name')
        else:
            self.path_p = self.googledrive_directory_p + '\\' + self.namefolder_p + '\\' + 'Data' + '\\' + self.date_p + '\\' + 'Keithley for Resistance'
            # Create a folder at this address
            if not os.path.isdir(self.path_p):
                os.makedirs(self.path_p)
            # This the name of the file
            self.name_p = self.googledrive_directory_p + '\\' + self.namefolder_p + '\\' + 'Data' + '\\' + self.date_p + '\\' + 'Keithley for Resistance' + '\\' + self.file_name_p + self.type_p
            
            parameter_txt = self.googledrive_directory_p + '\\' + self.namefolder_p + '\\' + 'Data' + '\\' + self.date_p + '\\' + 'Parameter.txt'
            f = open(parameter_txt, 'w')
            f.write('The parameters of the sweep voltage process are shown blow.' + '\n')
            f.write('Start Voltage' + self.divide_p + self.ui.lineEdit_start_voltage.text() + self.divide_p + 'mV' + '\n')
            f.write('First Peak Voltage' + self.divide_p + self.ui.lineEdit_first_voltage.text() + self.divide_p + 'mV' + '\n')
            f.write('Second Peak Voltage' + self.divide_p + self.ui.lineEdit_second_voltage.text() + self.divide_p + 'mV' + '\n')
            f.write('End Voltage' + self.divide_p + self.ui.lineEdit_end_voltage.text() + self.divide_p + 'mV' + '\n')
            f.write('Time Step' + self.divide_p + self.ui.lineEdit_voltage_time_measurements.text() + self.divide_p + 's' + '\n')
            f.write('Software Time Step' + self.divide_p + self.ui.lineEdit_steps_soft.text() + self.divide_p + '1' + '\n')
            f.close()
            
            f = open(self.name_p, 'w')
            # The beginning of the file is name and time
            f.write('Name' + self.divide_p + self.namefolder + '\n')
            f.write('Time' + self.divide_p + str(now) + '\n')
            f.write('\n')
            
            # First part of the file is labels and parameters
            f.write('Label' + self.divide_p + 'Parameter' + self.divide_p + 'Unit' + '\n')
            f.write('Sweep Source' + self.divide_p + self.Sweep + '\n')
            f.write('Points of Probe' + self.divide_p + str(self.Probe) + '\n')
            if self.Sweep == 'Voltage':
                f.write('Start Voltage' + self.divide_p + str(self.ui.lineEdit_start_voltage.text()) + self.divide_p + 'mV' + '\n')
                f.write('Frist Peak Voltage' + self.divide_p + str(self.ui.lineEdit_first_voltage.text()) + self.divide_p + 'mV' + '\n')
                f.write('Second Peak Voltage' + self.divide_p + str(self.ui.lineEdit_second_voltage.text()) + self.divide_p + 'mV' + '\n')
                f.write('End Voltage' + self.divide_p + str(self.ui.lineEdit_end_voltage.text()) + self.divide_p + 'mV' + '\n')
                if (self.ui.radioButton_software.isChecked() == True):
                    f.write('Steps Number' + self.divide_p + str(self.ui.lineEdit_steps_soft.text()) + self.divide_p + '\n')
                elif (self.ui.radioButton_hardware.isChecked() == True):
                    f.write('Steps Number' + self.divide_p + str(self.ui.lineEdit_steps_hard.text()) + self.divide_p + '\n')
                f.write('Current Limit' + self.divide_p + str(self.ui.lineEdit_current_limit.text()) + self.divide_p + 'uA' + '\n')
                f.write('Time Step' + self.divide_p + str(self.ui.lineEdit_voltage_time_measurements.text()) + self.divide_p + 's' + '\n')
                f.write('Resistance with the First Period Voltage' + self.divide_p + str(self.resistance_first) + self.divide_p + self.resistance_first_scale[1] + '\n')
                f.write('Resistance with the Second Period Voltage' + self.divide_p + str(self.resistance_second) + self.divide_p + self.resistance_second_scale[1] + '\n')
                f.write('Resistance with the Third Period Voltage' + self.divide_p + str(self.resistance_third) + self.divide_p + self.resistance_third_scale[1] + '\n')
                
                f.write('\n')
                f.write('Collected data' + '\n')
                f.write('Time' + self.divide_p + 'Voltage' + self.divide_p + 'Current' + self.divide_p + 'First Period Time' + self.divide_p + 'First Period Voltage' + self.divide_p +
                        'Current' + self.divide_p + 'Second Period Time' + self.divide_p + 'Second Period Voltage' + self.divide_p + 'Current' + self.divide_p + 'Third Period Time' +
                        self.divide_p + 'Third Period Voltage' + self.divide_p + 'Current' + self.divide_p + '\n')
                f.write('s' + self.divide_p + 'Volts' + self.divide_p + 'Amps' + self.divide_p + 's' + self.divide_p + 'Volts' + self.divide_p + 'Amps' +self.divide_p + 's' +self.divide_p + 'Volts' +
                        self.divide_p + 'Amps' + self.divide_p + 's' + self.divide_p + 'Volts' + self.divide_p + 'Amps' + '\n')
                for i in range(0, len(self.Time)):
                    full_data = str(self.Time[i]) + self.divide + str(self.voltage_values[i]) + self.divide + str(self.current_values[i])
                    try:
                        first_data = str(self.first_Time[i]) + self.divide + str(self.first_voltage[i]) + self.divide + str(self.first_current[i])
...
class Collect_data(QThread):
    ...
    def input(self, inst, sweep_array, sweep_array_first, sweep_array_second, sweep_array_third, sweep_lim, read_lim, wait_time, ui, sweep, probe):
        self.inst  = inst
        self.sweep_array = sweep_array
        self.sweep_array_first = sweep_array_first
        self.sweep_array_second = sweep_array_second
        self.sweep_array_third = sweep_array_third
    ...
    def stop(self):
        self.sweep_stop.stop_check = True
            
    def run(self):
        from datetime import datetime
        now_time = datetime.now()
        current_time = '%s:%s:%s' % (now_time.hour, now_time.minute, now_time.second)
        data = []
        #data[0]
        data.append(str(self.ui.lineEdit_run_name.text()))
        #data[1]
        ...
        if self.sweep == "VOLTAGE":
            ...
            #data[12]
            data.append(best_fit_line_first)
            ...
            #data[13]
            data.append(voltage_values[:length_first])
            #data[14]
            data.append(current_values[:length_first])
            #data[15]
            data.append(voltage_scale[1])
            #data[16]
            data.append(current_scale[1])
            ...
            #data[17]
            data.append(best_fit_line_second)
            ...
            #data[18]
            data.append(voltage_values[length_first:length_first + length_second])
            #data[19]
            data.append(current_values[length_first:length_first + length_second])
            #data[20]
            data.append(voltage_scale[1])
            #data[21]
            data.append(current_scale[1])
            ...
            #data[22]
            data.append(best_fit_line_third)
            ...
            #data[23]
            data.append(voltage_values[length_first + length_second:])
            #data[24]
            data.append(current_values[length_first + length_second:])
            #data[25]
            data.append(voltage_scale[1])
            #data[26]
            data.append(current_scale[1])
            ...
            #data[28]
            data.append(voltage_values_double)
            #data[29]
            data.append(voltage_scale[1])
            #data[30]
            data.append(self.ui.lineEdit_start_voltage.text())
            #data[31]
            data.append(self.ui.lineEdit_first_voltage.text())
            #data[32]
            data.append(self.ui.lineEdit_second_voltage.text())
            #data[33]
            data.append(self.ui.lineEdit_end_voltage.text())
            #data[34]
            data.append(self.ui.lineEdit_voltage_time_measurements.text())
            #data[35]
            data.append(self.ui.lineEdit_steps_soft.text())
            #data[36]
            data.append(str(now_time))
            #data[37]
            if (self.ui.radioButton_software.isChecked() == True):
                data.append('Voltage')
            #data[38]
            if (self.ui.radioButton_2_point.isChecked() == True):
                data.append('2')
            elif (self.ui.radioButton_4_point.isChecked() == True):
                data.append('4')
            
            return_data = [voltage_values, current_values, resistance_first, resistance_second, resistance_third, scaled_best_fit_first, scaled_best_fit_second, scaled_best_fit_third, voltage_scale, current_scale, resistance_first_scale, resistance_second_scale, resistance_third_scale, time, length_first, length_second, data]
