...
class sweep_stop():
    ...
    def voltage_sweep_stop(self, inst, Voltage, voltage_ascending, voltage_lim, current_lim, wait_time, probe = "4", ui = False, emit = None):
        length_ascending = len(voltage_ascending)
        length_whole = len(Voltage)
        time_difference = 0
        item = 0
        ...
        ui.mplwidget_left.figure.clear()
        while True:
            if self.continue_check == True:
                ...
                axes_left = ui.mplwidget_left.figure.add_subplot(111)
                axes_left.set_title('Voltage vs.Time')
                axes_left.set_ylabel('Voltage (mV)')
                axes_left.set_xlabel('Time (s)')
                number = 0
                start_time = time.time()
                ...
                axes_left.set_ylabel('Voltage (mV)')
                axes_left.set_xlabel('Time (s)')
                axes_left.plot(Time, x_double_value, marker = '.', linestyle = '-')
                if emit != None:
                    emit(SIGNAL("plot"))
